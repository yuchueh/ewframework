...
func (c *Controller) viewPath() string {
	if c.ViewPath == "" {
		return config.BConfig.WebConfig.ViewsPath
	}
	return c.ViewPath
}
...
func (c *Controller) CustomAbort(status int, body string) {
	...
	if _, ok := context.ErrorMaps[body]; ok {
...
func (c *Controller) URLFor(endpoint string, values ...interface{}) string {
	...
	if endpoint[0] == '.' {
		return ewtemplate.URLFor(reflect.Indirect(reflect.ValueOf(c.AppController)).Type().Name()+endpoint, values...)
	}
	return ewtemplate.URLFor(endpoint, values...)
}
...
func (c *Controller) ServeJSON(encoding ...bool) {
	...
	if config.BConfig.RunMode == utils.PROD {
		hasIndent = false
	}
...
func (c *Controller) ParseForm(obj interface{}) error {
	return ewtemplate.ParseForm(c.Input(), obj)
}
...
func (c *Controller) SessionRegenerateID() {
	...
	c.CruSession = session.GlobalSessions.SessionRegenerateID(c.Ctx.ResponseWriter, c.Ctx.Request)
...
func (c *Controller) XSRFToken() string {
	if c._xsrfToken == "" {
		expire := int64(config.BConfig.WebConfig.XSRFExpire)
		if c.XSRFExpire > 0 {
			expire = int64(c.XSRFExpire)
		}
		c._xsrfToken = c.Ctx.XSRFToken(config.BConfig.WebConfig.XSRFKey, expire)
	}
	return c._xsrfToken
}

...
func serverStaticRouter(ctx *context.Context) {
	...
	if forbidden {
		context.Exception(403, ctx)
		return
	}

	if filePath == "" || fileInfo == nil {
		if config.BConfig.RunMode == utils.DEV {
			logs.Warning("Can't find/open the file:", filePath, err)
		}
	...
	var enableCompress = config.BConfig.EnableGzip && isStaticCompress(filePath)
	...
	if err != nil {
		if config.BConfig.RunMode == utils.DEV {
			logs.Warning("Can't compress the file:", filePath, err)
		}
		http.NotFound(ctx.ResponseWriter, ctx.Request)
		return
	}
...
func isStaticCompress(filePath string) bool {
	for _, statExtension := range config.BConfig.WebConfig.StaticExtensionsToGzip {
...
func searchFile(ctx *context.Context) (string, os.FileInfo, error) {
	...
	if requestPath == "/favicon.ico" || requestPath == "/robots.txt" {
		...
		for _, staticDir := range config.BConfig.WebConfig.StaticDir {
	...
	for prefix, staticDir := range config.BConfig.WebConfig.StaticDir {
...
func lookupFile(ctx *context.Context) (bool, string, os.FileInfo, error) {
	...
	return !config.BConfig.WebConfig.DirectoryIndex, fp, fi, err
}

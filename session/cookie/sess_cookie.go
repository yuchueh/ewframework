...
import (
	...
	"github.com/yuchueh/ewframework/utils/encddecd"
)
...
func (st *CookieSessionStore) SessionRelease(w http.ResponseWriter) {
	encodedCookie, err := encddecd.EncodeCookie(cookiepder.block, cookiepder.config.SecurityKey, cookiepder.config.SecurityName, st.values)
	if err == nil {
...
func (pder *CookieProvider) SessionInit(maxlifetime int64, config string) error {
	...
	if pder.config.BlockKey == "" {
		pder.config.BlockKey = string(encddecd.GenerateRandomKey(16))
	}
	if pder.config.SecurityName == "" {
		pder.config.SecurityName = string(encddecd.GenerateRandomKey(20))
	}
...
func (pder *CookieProvider) SessionRead(sid string) (session.Store, error) {
	maps, _ := encddecd.DecodeCookie(pder.block,
...
func (pder *CookieProvider) SessionRegenerate(oldsid, sid string) (session.Store, error) {
	return nil, nil
}
